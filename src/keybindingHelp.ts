...
export const MODE_DESCRIPTIONS: Record<InputMode, string> = {
  ...
  VISUAL_MULTI: '離れた単語や下線を1つずつ追加・解除し、非連続の下線を作成するための選択モードです。',
  ARROW: '標識の付いた始点から、修飾矢印または同格接続をつなぐ相手を選択しているモードです。',
  BORDER: '単語や要素の間にある境目を選び、疑似トークンや括弧の挿入・削除を行うモードです。',
  FORM: '選択中の単語・疑似トークン・下線に、原形や過去形などのformを文字列で入力するモードです。',
  INSERT: '現在の組の英文全体をテキスト入力で新規作成または編集しているモードです。',
  TRANSLATION: '現在の英文に対応する訳文をテキスト入力で編集しているモードです。',
  PSEUDO_INPUT: '境目に補った語を疑似トークンとして追加するか、既存の疑似トークンを編集しているモードです。',
  MARKER_INPUT: '定型標識にない文字列も含め、選択中の要素に付ける標識を自由入力しているモードです。',
  MARKER_SEQUENCE: '定型標識入力を明示的に開始するか先頭文字を入力した後、標識の文字列を受け付ける専用の連続入力モードです。',
  DIALOG: '設定画面などのモーダルダイアログを開いて、項目やボタンを操作しているモードです。',
  MENU: '各組の操作メニュを開き、メニュ項目を移動・選択しているモードです。',
};
